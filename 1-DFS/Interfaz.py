...
# DFS sobre grafos
from DFS import dfs, reconstruir_camino as reconstruir_camino_grafo
...
class AppDFS(tk.Tk):
    def __init__(self):
        ...
        self.btn_iniciar = tk.Button(self, text="Iniciar DFS", command=self.iniciar)
    ...
    def iniciar(self):
        ...
        if PROBLEMAS[problema] == "jarras":
            ...
            if resultado:
                self.pasos = reconstruir_camino_grafo(resultado, inicio, objetivo)
                self.metricas = (f"\n--- Métricas ---\n"
                                 f"Tiempo de ejecución: {fin_tiempo - inicio_tiempo:.6f} s\n"
                                 f"Memoria máxima usada: {memoria_max/1024:.2f} KB")
            else:
                self.pasos = ["No se encontró camino"]
    ...
    def mostrar_paso(self):
        ...
        if self.pasos:
            ...
            # Si es el último paso, mostrar métricas y cambiar botón
